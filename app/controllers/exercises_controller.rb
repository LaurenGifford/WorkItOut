class ExercisesController < ApplicationController
  ...
  def update
    if exercise_params[ :category_id ].empty?
      newly_created_category = Category.create( exercise_params[ :category_attributes ] )
      flash[ :messages ] = newly_created_category.errors.full_messages unless newly_created_category.valid?
      @exercise.update( category_id: newly_created_category.id, name: exercise_params[ :name ], difficulty: exercise_params[ :difficulty ], muscle_group: exercise_params[ :muscle_group ], url: exercise_params[ :url ], instructions: exercise_params[ :instructions ] )
    else
      @exercise.update( category_id: exercise_params[ :category_id ], name: exercise_params[ :name ], difficulty: exercise_params[ :difficulty ], muscle_group: exercise_params[ :muscle_group ], url: exercise_params[ :url ], instructions: exercise_params[ :instructions ] )
    end
    if @exercise.valid?
      redirect_to exercise_path( @exercise )
    else
      flash[ :messages ] ||= @exercise.errors.full_messages
